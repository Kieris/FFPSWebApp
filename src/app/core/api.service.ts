...
export class ApiService {
  ...
  skillchains = ['None, Light, Darkness, Gravitation, Fragmentation, Distortion, Fusion, Compression, Liquefaction, Induration, Reverberation, Transfixion, Scission, Detonation, Impaction'];
  attType = ['None', 'Physical', 'Magical', 'Ranged', 'Special', 'Breath'];
  dmgTypes = ['None', 'Piercing', 'Slashing', 'Blunt', 'Hand-to-Hand', 'Elemental', 'Fire', 'Ice', 'Wind', 'Earth', 'Lightning', 'Water', 'Light', 'Dark' ];
  allegiance = ['Mob', 'Player', 'Sandoria', 'Bastok', 'Windurst'];

  constructor(private http: HttpClient) { }


  public sendGetAHItem(item: string){
    return this.http.get(environment.API + "ah/items/" + item);
  }

  public sendGetItems(item: string){
    return this.http.get(environment.API + "items/" + item);
  }

  public sendGetItemDetails(item: number){
    return this.http.get(environment.API + "items/item/dets/" + item);
  }

  public sendGetItemRecipes(item: number){
    return this.http.get(environment.API + "recipes/ingred/" + item);
  }

  public sendGetItemMerchants(item: number){
    return this.http.get(environment.API + "guildstore/" + item);
  }

  public sendGetItemById(item: number){
    return this.http.get(environment.API + "items/item/" + item);
  }

  public sendGetAHPlayer(name: string) {
    return this.http.get(environment.API + "ah/player/" + name);
  }

  public sendGetChar(name: string) {
    return this.http.get(environment.API + "chars/" + name);
  }
  
  public sendGetZoneEvent(id: number){
    return this.http.get(environment.API + "events/zones/" + id);
  }

  public sendGetZoneMobs(id: number){
  ...
  public sendGetZones(){
    return this.http.get(environment.API + "zones");
  }

  public sendGetMob(name: string){
    return this.http.get(environment.API + "mobs/mob/" + name);
  }
  ...
  public sendGetFishMob(id: string, zid: number){
    return this.http.get(environment.API + "fish/mob/" + id + "/" + zid);
  }

  public sendGetTraitsByJob(id: number){
    return this.http.get(environment.API + "traits/job/" + id);
  }

  public sendGetMiscNotes(id: string, type: string){
    return this.http.get(environment.API + "misc/" + type + "/" + id);
  }

  public sendGetAbilitiesByJob(id: number){
    return this.http.get(environment.API + "jobs/" + id + "/abilities");
  }

  public sendGetSkillRanksByJob(id: number){
    return this.http.get(environment.API + "skills/job/" + id);
  }

  public sendGetSkillRanksByLevel(level: number, rank: number){
    return this.http.get(environment.API + "skills/job/" + level +"/" +rank);
  }

  public sendGetSpellsByJob(job: number){
    return this.http.get(environment.API + "spells/job/" + job);
  }

  public sendGetAFByJob(job: number){
    return this.http.get(environment.API + "armor/job/" + job);
  }

  public sendGetWSByType(id: number){
    return this.http.get(environment.API + "ws/" + id);
  }

  public sendGetSCByType(id: number){
    return this.http.get(environment.API + "scmap/" + id);
  }

  public sendGetWSBySCType(id: number){
    return this.http.get(environment.API + "ws/sc/" + id);
  }

  public sendGetMeritsByJob(id: number){
    return this.http.get(environment.API + "merits/job/" + id);
  }

  public sendGetPupSpells(){
    return this.http.get(environment.API + "pup/spells");
  }
  ...
  public sendGetCraftRecipes(id: string, lvl1: number, lvl2: number){
    return this.http.get(environment.API + "crafts/recipes/" + id + "/" + lvl1 + "/" + lvl2);
  }

  public sendGetFishByLevel(id: string, lvl1: number, lvl2: number, incID){
    return this.http.get(environment.API + "fishing/fish/" + id + "/" + lvl1 + "/" + lvl2 + "/" + incID);
  }

  public sendGetFishDetails(id: number){
    return this.http.get(environment.API + "fish/dets/" + id);
  }

  public sendGetFishAreas(id: number){
    return this.http.get(environment.API + "fish/areas/" + id);
  }
  /*public getJsonData() {
    this.http.get("../../assets/FFXI_Events-master/beadeaux.json").toPromise().then(data =>{
      console.log(data);
    })
  }*/
}
